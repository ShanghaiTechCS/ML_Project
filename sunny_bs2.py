...
def regression_mlp(data_dict=None):
    """
    This baseline can estimate the customer whether responded.
    And the ground-truth is the (responded_target \cap (profit_target>30))
    ...
    regression_pro = MLPRegressor(hidden_layer_sizes=(128, 128, 128), activation='relu', solver='adam', alpha=0.001,
                                  batch_size='auto', learning_rate='adaptive', max_iter=1000000, shuffle=True,
                                  tol=1e-48, momentum=0.99, verbose=True, warm_start=True, learning_rate_init=0.00001)
    regression_pro.fit(training_data, training_gt)
    score_train = regression_pro.predict(training_data)
    mse_train = np.mean((score_train - training_gt) ** 2)
    score_val = regression_pro.predict(val_data)
    ...
    print('The train MSE: ', mse_train, 'The val MSE: ', mse_val)


def baseline2(best_cls_alpha, best_reg_alpha, data_dict):
    training_data = data_dict['training_data']
    one_index = np.array(data_dict['training_gt'][:, 1] > 30, dtype=np.int16)
    data_dict['training_gt'][:, 0] = data_dict['training_gt'][:, 0] * one_index
    ...
    reg_result_train = reg.predict(X=training_data)
    reg_result_val = reg.predict(X=val_data)
    ...
    mse = np.mean((reg.predict(X=val_data_profit) - val_gt_profit) ** 2)

    total_acc_train, rec_acc_train = recall_cls(cls_pred=cls_result_train, cls_gt=train_gt_cls)
    total_acc_val, rec_acc_val = recall_cls(cls_pred=cls_result_val, cls_gt=val_gt_cls)

    print('validation:', 'acc:', acc, 'MSE:', mse)
    print('total_acc_train: %.3f, rec_acc_train:%.3f, total_acc_val:%.3f, rec_acc_val:%.3f'
          % (total_acc_train, rec_acc_train, total_acc_val, rec_acc_val))
